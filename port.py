...
def openports(ip):
    ...
    if 80 not in op:
        print('\033[33mfor ip::',ip,'no open port 80.\ntry restart.')
def openports2():
    ip=input('IP: ')
    port=int(input('PORT: '))
    for lol in[port]:
        try:
            ...
            sock.settimeout(0.2)
            sock.connect((ip, lol))
            print('\033[32mport:: %s' % lol, ':: open.')
            sock.close()
        except:
            print('\033[31mport:: %s' % lol, ':: close.')
            print('\033[32mdone.')
print('''
[1] scan standart ports.
[2] scan certain port. 
